#TODO: 
# 1. Lấy danh sách paper từ arXiv theo category hoặc query tuỳ biến 
# 2. Lấy paper theo arXiv ID
# 3. Tải PDF về cache cục bộ kèm retry/backoff
# 4. Tuân thủ rate-limit của arXiv 
import asyncio
import logging 
import time
...
import httpx
from controller.config import ArxivSettings
from controller.exceptions import ArxivAPIException, ArxivParseError, ArxivAPITimeoutError, ArxivAPIRateLimitError, PDFDownloadException, PDFDownloadTimeoutError, PDFDownloadError
from schemas.paper import ArxivPaper
...
class ArxivClient:
    ...
    async def fetch_papers(
        ...
        # Create safe urlencode function that doesn't encode special chars
        safe = "+:[]"
        ...
        try:
            ...
            # Add rate limiting delay between all requests (arXiv recommends no more than 1 request every 3 seconds)
            if self._last_request_time is not None:
                time_since_last = time.time() - self._last_request_time
                if time_since_last < self.rate_limit_delay:
                    delay = self.rate_limit_delay - time_since_last
                    logger.debug(f"Rate limiting in effect, sleeping for {delay:.2f} seconds")
            ...
            paper = self._parse_response(xml_data)
            logger.info(f"Fetched {len(paper)} papers from arXiv")
            return paper
    ...
    async def fetch_papers_with_query(
        ...
        """
        Fetch papers from arXiv matching a custom search query.

        Args:
            search_query: Custom search query string (e.g. "quantum computing")
            max_results: Maximum number of papers to fetch (uses settings default if None)
            start: Starting index for pagination
            sort_by: Sort criteria (submittedDate, lastUpdatedDate, relevance)
            sort_order: Sort order (ascending, descending)
    ...
    async def fetch_paper_by_id(self, arxiv_id: str) -> Optional[ArxivPaper]:
        """
        Fetch a single paper from arXiv by its arXiv ID.

        Args:
            arxiv_id: The unique identifier for the paper on arXiv (e.g. "2101.00001")

        Returns:
            An ArxivPaper object if found, else None
        """
